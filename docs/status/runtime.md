...
- Result: `Ran 26 tests in 7.274s` / `OK`. Default fixture `job_01` is `error` / `solver body is not implemented`. Injected `solve` covers feasible, infeasible, and timed_out. Crash, invalid stdout, and sleep still use `PLANES_SOLVER_ARGV` to the placeholder.
