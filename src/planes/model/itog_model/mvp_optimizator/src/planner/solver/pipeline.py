...
def _generate_all_swaths(
    ...
    """Генерирует полосы для всех областей при данном угле θ."""
    catalog = get_default_catalog()
    swaths_by_area: dict[str, list] = {}
    h_agl_by_area: dict[str, float] = {}

    for area in mission.areas:
        # Для MVP берём камеру первого борта
        uav = mission.uavs[0]
...
def _solve_for_uav(
    ...
    """Решает маршруты для одного борта (возможно, несколько вылетов)."""
...
def _build_candidate(
    ...
    """Собирает Candidate из маршрутов."""
    if not all_routes:
        return None

    C_max_s = max(r.T_total_s for r in all_routes)
    flight_hours_s = sum(r.T_air_s for r in all_routes)
    energy_total_wh = sum(r.E_wh for r in all_routes)
    n_uavs_used = len({r.uav_id for r in all_routes})
...
def run_one_angle(
    ...
    """Прогон одного угла θ. Возвращает Candidate или None."""
    catalog = get_default_catalog()
    ...
    # 5. Маршрутизация
    all_routes: list[Route] = []
    wind_mps = mission.params.wind.speed_mps
...
def lns_improve(
    ...
    """
    Упрощённый LNS: меняет порядок полос местами и пересобирает маршруты.
    Для MVP — заглушка (возвращает как есть).
    """
    return candidate
...
def run_mission(fixtures_dir: str | Path, output_dir: str | Path) -> Report:
    ...
    for theta in mission.params.angles_deg:
        ...
        # Генерируем полосы один раз на угол
