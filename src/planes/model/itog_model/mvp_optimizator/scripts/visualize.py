...
Запуск:
    python3 scripts/visualize.py tests/fixtures out/visualize/routes.html
"""
...
def build_map(mission, swaths_by_id: dict, candidate) -> folium.Map:
    vpp0 = mission.vpps[0]
    m = folium.Map(location=[vpp0.lat, vpp0.lon], zoom_start=15, tiles=None)
    ...
    for area in mission.areas:
        ...
        folium.Polygon(
            locations=latlngs,
            color="orange", weight=2, fill=True,
            fill_color="yellow", fill_opacity=0.15,
            popup=f"Area: {area.id}",
            tooltip=area.id,
        ).add_to(fg_areas)
    fg_areas.add_to(m)
    ...
    for idx, (uav_id, routes) in enumerate(by_uav.items()):
        ...
        for r in sorted(routes, key=lambda x: x.flight_index):
            ...
            # NEW: waypoints если есть, иначе fallback
...
if __name__ == "__main__":
    ...
    out_html = sys.argv[2] if len(sys.argv) > 2 else "out/visualize/routes.html"
